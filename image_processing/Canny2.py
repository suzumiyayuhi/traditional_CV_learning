...
def SobelOperator(img, threshold):
    ...
    for i in range(1, img.shape[0] - 1):
        for j in range(1, img.shape[1] - 1):
            ...
            temSum_x = abs(temSum_x)
            temSum_y = abs(temSum_y)
            temSum = abs(temSum_x) + abs(temSum_y)
...
def CannyEdgeDetection(img, kernelSize, sigma):
    out = img
    lev = img
    theta = img

    img = GaussFilter(img, 3, 5)
    [img_x, img_y, img] = SobelOperator(img, 150)
...
''''''
temImg = cv2.imread("images/coins_240p_gray.jpg")
temImg = cv2.cvtColor(temImg, cv2.COLOR_BGR2GRAY)
CannyEdgeDetection(temImg, 3, 5)
